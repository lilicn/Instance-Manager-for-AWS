  ...
  // format
	...
	public void launch(String command) throws Exception {
		...
		if(! this.launchInstancePattern.matcher(command).matches()) {
			System.out.println("Input format is not correct. " + Utils.FORMAT_COMMAND_LAUNCH_INSTANCE_VPC);
			return;
		}
		...
		while(st.hasMoreTokens()) {
			...
			if(tag.equalsIgnoreCase("-i")) {
				...
				request.withSubnetId(value);
			}
		}
		
		
		// -s subnetID -i ami -t instanceType -k keyName -z zone -c minCount maxCount
		RunInstancesResult result = AWSVPCManager.ec2.runInstances(request);
	...
	public void start(String command) {
		...
		StartInstancesResult result = AWSVPCManager.ec2.startInstances(request);
	...
	public void stop(String command) {
		...
		request.withInstanceIds(instanceList);
		StopInstancesResult result = AWSVPCManager.ec2.stopInstances(request);
	...
	public void reboot(String command) {
		...
		request.withInstanceIds(instanceList);
		AWSVPCManager.ec2.rebootInstances(request);
	...
	public void terminate(String command) {
		...
		request.withInstanceIds(instanceList);
		TerminateInstancesResult result = AWSVPCManager.ec2.terminateInstances(request);
	...
	public void describeInstances() {
		...
		for(Reservation r : AWSVPCManager.ec2.describeInstances().getReservations()) {
